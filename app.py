"""
ETagMF Recommender System — Flask Application
==============================================
Paper: "A Genetic-Algorithms Matrix-Factorization Based Recommender
        System Model Using Tags", IEEE Access 2025
DOI  : 10.1109/ACCESS.2025.3550296

Paper Concept → Our Adaptation
─────────────────────────────────────────────────────────────────────
Paper uses MovieLens "tag genome":
  P (item × tag)  — relevance of each tag to each movie  [0,1]
  Q (user × tag)  — user affinity for each tag  [0,1]  ← GA optimises this
  R̂ = Q × Pᵀ      — predicted rating matrix

Our HW data has no explicit tags → we build a "pseudo tag-genome" P
from product features (category one-hot + normalised price +
behaviour-derived popularity), giving every product a unique
multi-dimensional vector in [0,1] — exactly like tag-genome scores.

GA role (same as paper):
  - Chromosome  = full Q matrix (users × tags)
  - Fitness     = RMSE(R, R̂) where R̂ = Q × Pᵀ
  - Selection   = Roulette Wheel (1/RMSE weighting)
  - Crossover   = single random row swap between two parents
  - Mutation    = gene replaced by random value in [0,1] with prob Pm
  - Elitism     = best chromosome always carried to next generation

Data files (place in data/ folder):
  users.xlsx    : user_id, age, location
  products.xlsx : product_id, category, price
  ratings.xlsx  : user_id, product_id, rating (1–5)
  behavior.xlsx : user_id, product_id, viewed, clicked, purchased

Only external libraries: flask, openpyxl
"""
...
def compute_rmse(Q, P, R, user_ids):
    ...
    for (uid, pid), actual in R.items():
        if uid not in uid_idx or pid not in P:
            continue
        # Normalise actual rating to [0, 1] to match Q·Pᵀ scale
...
def run_ga(P, R, user_ids, Np=20, Ni=100, Pc=0.8, Pm=0.05, seed=42):
    ...
    # ── Steps 2–3: Evolve for Ni generations ─────────────────────────
    for _ in range(Ni):
        next_pop  = [[row[:] for row in best_Q]]   # elitism: carry best forward
        next_fit  = [best_rmse]
...
def run_endpoint():
    ...
    # Top-N recommendations for the chosen user
    recs = recommend(best_Q, P, R, user_ids, products, uid, top_n)

    # User's actual ratings (for reference display)
    user_ratings = []
    ...
    # Learned Q values for the chosen user (tag affinities)
    uid_idx  = {u: i for i, u in enumerate(user_ids)}
    q_values = []
    if uid in uid_idx:
